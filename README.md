...
It covers why a **32B or 235B VL does not fit** beside a ~102 Gi LLM, why the vision file is a **separate `mmproj` GGUF** (omit it and you get a text-only server), why llama.cpp warns that Qwen-VL needs **`--image-min-tokens 1024`**, why **four parallel slots** eat the last of unified memory, why **both services loading at boot** can spike past 121 Gi, why a **PowerShell `$(…)`** in an SSH command never reaches Linux, and why a Wikimedia test JPEG returns **HTTP 400** with no User-Agent.
...
```bash
scp llama-server-vl-start.sh llama-server-vl.service install-vl-systemd.sh <user>@100.x.y.z:/tmp/
ssh <user>@100.x.y.z 'sed -i "s/\r$//" /tmp/llama-server-vl-start.sh /tmp/llama-server-vl.service /tmp/install-vl-systemd.sh
  mkdir -p ~/inference
...
The unit's extra `ExecStartPre` polls `http://100.x.y.z:8000/v1/models` for the string `deepseek`, every 5 s, up to 5 minutes, then starts Qwen-VL anyway. `TimeoutStartSec=900` covers that wait plus the ~10 s VL load. `After=llama-server.service` only orders **process spawn**, not "model finished loading" — `Type=simple` considers the LLM started the moment `llama-server` exists, which is ~8 minutes before `:8000` returns 200. The curl loop is what actually serializes the two mappings.

### Pitfall #5 — both services `WantedBy=default.target` load weights at once

Symptom you are trying to avoid: cold boot maps ~102 Gi (LLM) and ~10 Gi (VL) concurrently, on 121 Gi usable, plus OS. Not observed as an OOM in this session (Qwen-VL was started **after** the LLM was already resident). Correction: the `ExecStartPre` wait. **Not proven with a real power cycle of the two-service stack** — see Known limitations. The wait is in the unit that is enabled; a two-model reboot test has not been run.
...
| Cold boot maps both weight files at once | `Type=simple` does not wait for LLM 200 | `ExecStartPre` curl-loop on `:8000` |
...
- **Two-model cold boot is designed, not proven.** Linger + the `ExecStartPre` wait are in the enabled unit. A power-off / power-on cycle that proves **both** APIs return with zero SSH was run for the LLM alone (separate work); it has **not** been repeated with Qwen-VL also enabled.
