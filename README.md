...
**Where this fits**: after [headless setup](https://github.com/AI-Architect-Lab-333/dgx-spark-headless-setup) and [cross-host inference](https://github.com/AI-Architect-Lab-333/dgx-spark-cross-host-inference). [The blind circuit](https://github.com/AI-Architect-Lab-333/dsh-blind-circuit-windows) is what consumes both APIs on confidential files. To stop reloading the pair on every power button, and to free the GPU for simulation: [idle vs LLM boot profiles](https://github.com/AI-Architect-Lab-333/dgx-spark-idle-llm-profiles), then [headless MuJoCo](https://github.com/AI-Architect-Lab-333/dgx-spark-mujoco-headless-panda).
...
`After=llama-server.service` only orders **process spawn**. `Type=simple` treats the LLM as started the moment `llama-server` exists — about **8 minutes before** `:8000` returns 200. An HTTP 200 is still **not** enough: Qwen-VL that starts at that instant dies in `ggml_cuda_init` / `cudaSetDevice` with `CUDA error: out of memory` even while `free -h` shows ~19 Gi **available**. On this hardware the LLM process holds **~100 469 MiB** in `nvidia-smi --query-compute-apps`. `wait-ds4-ready.sh` waits for HTTP 200 **and** that GPU allocation ≥ 90 000 MiB, stable 60 s (delta 2048 MiB). The unit in this repo sets `Restart=no`. That is not a llama.cpp rule; it is what this box ran for the closing power cycle. Earlier, with `Restart=on-failure` and a wait that started Qwen-VL at the first HTTP 200, CUDA ABRT retried every few minutes, did not recover after a driver bump, and made the next `cudaSetDevice` worse. A cold boot that keeps `wait-ds4-ready.sh` but puts `Restart=on-failure` back was **not** repeated. `TimeoutStartSec=2400` covers the LLM load plus that plateau.
...
Then `systemctl --user is-active llama-server-vl` should be `active` and `NRestarts=0` — that is what the closing power cycle produced with **these files**. A dump every five minutes means `Restart=on-failure` is still loaded (the policy that looped in the earlier tests).
...
| VL core-dumps every 5 min after boot | `Restart=on-failure` after CUDA ABRT (measured with a too-early start) | This unit: `Restart=no`. Cold boot with GPU-wait **and** `on-failure` was not re-tested |
...
- **Two-model cold boot is proven** with the unit in this repo (`wait-ds4-ready.sh` + `Restart=no`; August 2026 power-off / button / both `/v1/models` 200 at ~12 min, zero SSH until then, `NRestarts=0`). A **curl-only** wait on the first HTTP 200, or a 180 s / first 300 s wall-clock sleep, still CUDA-OOMs. After a DGX OS / driver bump (observed: NVIDIA driver 580.173.02), a `Restart=on-failure` dump loop of 5 min retries did **not** recover; a single start once the LLM GPU allocation was stable did. Putting `Restart=on-failure` back on top of `wait-ds4-ready.sh` was not power-cycled again.
